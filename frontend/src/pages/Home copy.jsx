...
const Home = () => {
  ...
  // const checkEmailAvailability = debounce(async (email, setEmailStatus) => {
  //   try {
  //     const response = await axios.post('/api/check-email/', { email });
  //     setEmailStatus(response.data); // { available: true/false, message: "..."}
  //   } catch (error) {
  ...
  const handleChange = async (e, section, key, index = null) => {
    ...
    // if (key === "email") {
    //   if (!value.trim()) {
    //     setEmailStatus(null); // Clear feedback if the input is empty
    //     return;
    //   }
    ...
    try {
      ...
      setFormData((prevFormData) => {
        ...
        if (parsedData.portfolio) {
          updatedFormData.portfolio = parsedData.portfolio.map(
            (link, index) => ({
          ...
          <div className="section">
            ...
            <div className="input-container">
              ...
              <textarea
                ...
                // value={formData.skill_set.skills.join(', ')}
                value={formData.skill_set.skills[0]}
